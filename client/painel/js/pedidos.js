...
pedido.event = {
    ...
    init: () => {
        ...
        }, 100000000000); // atualiza a cada 10 segundos
...
pedido.method = {
    ...
    carregarModalDetalhes: (data, idpedido, cart) => {
        ...
        if (data.idtipoentrega == 1) {
            ...
            document.querySelector("#lblEndereco").innerText = `${data.endereco}, ${data.numero}, ${data.bairro}${data.complemento && data.complemento.length > 0 ? ` (${data.complemento})` : ''}`;
        ...
        order.forEach((e, i) => {
            ...
            console.log('taxaentrega=',data.taxaentrega)
        // valida se tem taxa 
        if (data.taxaentrega > 0 && data.idtipoentrega == 1) {
